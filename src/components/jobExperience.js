    ...
    <div className="flex items-start space-x-4 mb-8 p-4 bg-gray-800 rounded-lg shadow-lg">
      ...
      <div>
        ...
        {jobTitle.map((title, idx) => {
          // Get the skills for this role (fallback to empty array)
          const roleSkills = Array.isArray(skills) && Array.isArray(skills[idx])
            ? skills[idx]
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {roleSkills.map((skill, sIdx) => (
                    <span key={sIdx} className="bg-gray-700 text-gray-300 px-2 py-1 rounded-md text-sm">
                      {skill}
                    </span>
                  ))}
                </div>
